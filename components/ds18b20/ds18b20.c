...
#define MATCH_ROM	0x55  // Command to address all devices on the bus
...
#define TEMP_LSB        0
#define TEMP_MSB        1
#define HIGH_ALARM_TEMP 33
#define LOW_ALARM_TEMP  22
...
bool ds18b20_setResolution(uint8_t mode, const DeviceAddress tempSensorAddresses[],
		int numAddresses, uint8_t newResolution) {
	...
	if(mode == SKIP_ROM){
		ds18b20_readScratchPad(SKIP_ROM, tempSensorAddresses, scratchPad);
		*(scratchPad + 4) = newResolution;
		ds18b20_writeScratchPad(SKIP_ROM, 0, scratchPad);
	}
	...
	/*for (int i = 0; i < numAddresses; i++){
		...
		if (ds18b20_isConnected((DeviceAddress*) tempSensorAddresses[i], scratchPad)) {
			...
			if (scratchPad[CONFIGURATION] != newValue) {
				...
				ds18b20_writeScratchPad(SKIP_ROM, (DeviceAddress*) tempSensorAddresses[i], scratchPad);
			}*/
			// done
			success = true;
...
void ds18b20_writeScratchPad(uint8_t mode,
		const DeviceAddress *deviceAddress, const uint8_t *scratchPad) {
	if(mode == 0 || mode == SKIP_ROM){
		ds18b20_reset();
...
bool ds18b20_readScratchPad(uint8_t mode, const DeviceAddress *deviceAddress, uint8_t* scratchPad) {
...
bool ds18b20_isConnected(const DeviceAddress *deviceAddress, uint8_t *scratchPad) {// ...
	bool b = ds18b20_readScratchPad(SKIP_ROM, deviceAddress, scratchPad);
...
float ds18b20_get_temp(uint8_t mode, uint8_t* rom) {
  if(init==1){
    unsigned char check;
    char temp1=0, temp2=0;
      ...
      if(check==1)
      {
    	  if(mode == SKIP_ROM){
			...
			ds18b20_send_byte(GETTEMP);
			vTaskDelay(750 / portTICK_PERIOD_MS);
			check=ds18b20_RST_PULSE();
			ds18b20_send_byte(SKIP_ROM);
			ds18b20_send_byte(0xBE); //read scratch
